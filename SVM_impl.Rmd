...
#test set prediction
pred_test <- predict(classifier_rbf, newdata = test[-1])

mod.probs <- attr(pred_test, "decision.values")
mod.class <- predict(classifier_rbf, test[-1], type = "class")
mod.labels <- test$SeriousDlqin2yrs

mod.cm <- confusionMatrix(mod.labels,mod.class)
